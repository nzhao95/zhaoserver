...
impl TaskBmc {
    pub async fn create(_ctx: &Ctx, mm: &ModelManager, task_c: TaskForCreate) -> Result<i64> {
        base::create::<Self, TaskForCreate>(_ctx, mm, task_c).await
    }
    ...
    pub async fn update(_ctx: &Ctx, mm: &ModelManager, id: i64, task_u: TaskForUpdate) -> Result<()> {
...
mod tests {
    ...
    #[serial]
    #[tokio::test]
    async fn test_list_err_not_found() -> Result<()> {
        let mm = _dev_utils::init_test().await;
        let ctx = Ctx::root_ctx();
